...
> 原生 App 走[小米 MiMo](https://platform.xiaomimimo.com/)(`mimo-v2.5`,一个模型同时管文字与看图);
> 另两种实现(PopClip / Hammerspoon)仍走 DeepSeek。
>
> - **翻译**:方向自动判断——含中文 → 译成英文;其它语言 → 译成简体中文。
> - **解释**:用简体中文把选中的概念/代码直接讲明白(代码会拆解逻辑、点出语法与设计意图;
>   概念用大白话讲清是什么/为什么/怎么用)。结果**流式逐字弹出**(打字机),浮窗高度随内容平滑长高,
...
去 [platform.xiaomimimo.com](https://platform.xiaomimimo.com/) 申请一个小米 MiMo 的 Key,然后:

```sh
mkdir -p ~/.config/popdict
echo '你的-mimo-key' > ~/.config/popdict/mimo_key
chmod 600 ~/.config/popdict/mimo_key
```

Key 只保存在本机这个文件里,**不会上传到任何地方**(只在翻译/解释/看图时直接发给 MiMo 官方接口)。
...
- **划词翻译 / 解释**:在任意 App 里选中一段文字 → 旁边冒出「🌐 翻译」「💡 解释」→ 点哪个走哪个 →
  原地显示结果(可选中复制,点别处关闭)。解释为流式输出,会逐字弹出、浮窗随内容平滑长高,
